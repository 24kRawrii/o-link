...
olink._register('vehicleproperties', {
    ...
    GetVehicleProperties = function(vehicle)
        ...
        local primaryColor = colorPrimary
        local secondaryColor = colorSecondary
    ...
    SetVehicleProperties = function(vehicle, props)
        ...
        if props.windows then
            for i = 1, #props.windows do RemoveVehicleWindow(vehicle, props.windows[i]) end
        end

        if props.doors then
            for i = 1, #props.doors do SetVehicleDoorBroken(vehicle, props.doors[i], true) end
        end
        ...
        if props.brokenWheels then
            if type(props.brokenWheels) == 'number' then
                BreakOffVehicleWheel(vehicle, props.brokenWheels, false, true, true, false)
            else
        ...
        if props.doorLockStatus then SetVehicleDoorsLocked(vehicle, props.doorLockStatus) end
